# Headout AI Storefront — project context for Codex
...
- Prototype code lives in `artifacts/` (Vite storefront + Express api-server + Drizzle `lib/`) — being replaced by the Next.js build. `Codex-PROMPT-*.md` / `REPLIT-PROMPT-*.md` are historical build prompts.
